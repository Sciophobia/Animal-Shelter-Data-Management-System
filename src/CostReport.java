...
public class CostReport extends JFrame {
	...
	public CostReport(ArrayList<Expense> animals) {
		...
		Object[][] data = new Object[animals.size()][animals.size()];
		for (int i = 0; i < animals.size(); i++) {
			data[i][0] = animals.get(i).getName();
			data[i][1] = animals.get(i).getCost();
		}
		JLabel label = new JLabel("Furry Friends Animal Shelter");
		label.setFont(new Font("Sylfaen", Font.PLAIN, 23));
		label.setBounds(279, 11, 285, 31);
		getContentPane().add(label);
		JLabel lblInventoryReport = new JLabel("Cost Report");
		lblInventoryReport.setFont(new Font("Sylfaen", Font.PLAIN, 18));
		lblInventoryReport.setBounds(369, 38, 128, 31);
		getContentPane().add(lblInventoryReport);
		JScrollPane scrollPane = new JScrollPane();
		scrollPane.setBounds(38, 104, 760, 360);
		getContentPane().add(scrollPane);
		String[] columnNames = {"Expense Name", "Expense Cost ($)"};
		 ...
		 *a print out of exactly what the user sees on the JTable - the names of the animals in one column, and the cage numbers in the other.
		 */
		btnPrint.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {	
				...
				try {
					...
					MessageFormat footer = new MessageFormat("Cost Report of " + animals.get(0).getAnimalName());
		...
		JLabel lblNewLabel = new JLabel(animals.get(0).getAnimalName());
		lblNewLabel.setBounds(394, 65, 103, 14);
